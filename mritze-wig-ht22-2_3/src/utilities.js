...
function dishType(dish){
    ...
    let result = dish.dishTypes.find(isKnownTypeCB);
...
*/
function nameToOrderCB(dishA, dishB){
    ...
    // Find out the type.
    let A = dishType(dishA);
    let B = dishType(dishB);

    // Convert to integer.
    A = dishTypeRanking[A];
    B = dishTypeRanking[B];

    // Comparison resulting in a number used by the Array.sort() function.
    return A - B;
...
function sortDishes(dishes){
    return [...dishes].sort(nameToOrderCB);//TODO
}
...
function menuPrice(dishesArray){
    ...
    function priceFetcherCB(dish){   // placeholder
        return dish.pricePerServing;
    }

    function priceAdderCB(term1, term2){   // placeholder
        return term1 + term2;
    }
    ...
    let arrayOfPrices = dishesArray.map(priceFetcherCB);

    return arrayOfPrices.reduce(priceAdderCB, 0);
