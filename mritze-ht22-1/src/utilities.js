...
function isKnownTypeCB(type){
    ...
    if(dishTypeRanking[type] === 1 || dishTypeRanking[type] === 2 || dishTypeRanking[type] === 3) {
        return type;
    }
...
function menuPrice(dishesArray){
    ...
    function sumPriceCB(dishA, dishB) {
        return (dishA + dishB); 
    }
    ...
    return dishesArray.map(priceCB).reduce(sumPriceCB, 0); 
