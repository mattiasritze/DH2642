...
function renderIngredients(ingredientArray, people){
    function ingredientTableRowCB(ingr){
        return <tr key={ /* TODO what's a key? */ingr.id}><td>{ingr.name}</td> <td>{ingr.aisle}</td><td class="align-right">{(people*ingr.amount).toFixed(2)/* multiply by number of people! Display with 2 decimals, use a CSS classs to align right */
